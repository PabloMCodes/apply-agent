...
async function profile() {
  const [data,resumes,ai,applicationFields]=await Promise.all([api('/profile').catch(()=>({})),api('/resumes'),api('/settings/ai'),api('/profile/fields')]);
  const form=el('form');
...
async function review(id) {
  ...
  function paint(){
    ...
    if(ready){
      ...
      submit.disabled=!snapshot.can_submit;actions.append(submit,button('Close without submitting',async()=>{await send(`/applications/${id}/cancel`,'POST');toast('Closing the browser session…');},'secondary'));
